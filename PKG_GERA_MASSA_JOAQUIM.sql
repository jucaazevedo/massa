CREATE OR REPLACE PACKAGE PKG_GERA_MASSA_JOAQUIM AS
       ...
       l_valor_total_venda_parcelado number:=0;
       l_qtde_total_trans_parcelado number:=0;
   ...
   PROCEDURE INPUT_DADOS_10_20 (
    ...
    PROCEDURE GERACAO_MASSA_10_20_TE (p_numero_geracao_massa varchar2,l_i in out number);
    ...
    PROCEDURE LAYOUT_TE102000_CRITICAS (p_numero_geracao_massa number);
    PROCEDURE LAYOUT_TE102002_CRITICAS (p_numero_geracao_massa number );
...
CREATE OR REPLACE PACKAGE BODY PKG_GERA_MASSA_JOAQUIM AS
       ...
       begin
            ...
            insert into TBL_INPUT_MASSA_DADOS_B0_BZ
            (
...
PROCEDURE INPUT_DADOS (
       ...
       begin
            ...
            (
                ...
                TIPO_LAYOUT,
                NRO_CNPJ_CPF_TE05, 
                NRO_PONTO_VENDA_TE05,
                DSC_TIPO_PESSOA_TE05, 
                QTD_DIAS_LIQ_TRAN_TE05,
                COD_PRODUTO_TE05, 
                VLR_TRAN_TE05, 
                IND_MOVIMENTACAO,
                QTD_PARC_TRAN_TE05, 
                VLD_VENDA_TE05,
                COD_ADQUIRENTE_TE05, 
                COD_BANCO_EMISSOR_TE05,
                NRO_CARTAO_TE05, 
                NRO_MCC_PONTO_VENDA_TE05, 
                COD_BANDEIRA_TE05,
                DSC_TIPO_OPERACAO_TE05, 
                NRO_REFERENCIA, 
                VL_TAXA_EMBARQUE, 
                NRO_PARCELA
            )
            values
            (
                to_number(p_numero_geracao_massa), 
       ...
       begin
            ...
            into campo_15
            from TBL_INPUT_MASSA_DADOS_B0_BZ
            ...
            update TBL_INPUT_MASSA_DADOS_B0_BZ a
...
--                 l_cod_adquir varchar2(8):='00000025';
       begin
            ...
            campo_13 := lpad(to_char(l_qtde_total_trans_parcelado),8,'0');
            campo_14 := lpad(to_char(l_valor_total_venda_parcelado),15,'0');
 ...
 --           campo_12 := lpad(2,8,'0');
       ...
       begin
            ...
            if l_tipo_layout in ('TE05','TE25') then
            ...
            if l_tipo_layout IN ('TE06','TE26') then
            ...
            if l_tipo_layout = 'TE46' then
                l_valor_total_venda_parcelado := l_valor_total_venda_parcelado + to_number(valor_soma);
                l_qtde_total_trans_parcelado := l_qtde_total_trans_parcelado + 1;
            end if;
...
--                     where x.
       ...
       PROCEDURE LAYOUT_SUBTIPO_01 (p_numero_geracao_massa number,
                 ...
                 campo_17 char(3) := lpad('0',3,'0'); --Número da Parcela
       ...
       begin
            ...
            select substr(a.tipo_layout, 3, 2),
                   ...
                   lpad(nvl(a.ind_movimentacao, case when a.tipo_layout = 'TE46' then '1' else '0' end), 1, '0'),
       ...
       PROCEDURE LAYOUT_SUBTIPO_02 (p_numero_geracao_massa number,
                 ...
                 campo_10 char(19) := lpad(' ',19,' '); --Token PAN
                 campo_11 char(11) := lpad(' ',11,' '); --Token Requestor ID
                 campo_12 char(2) := lpad(' ',2,' '); --Token Assurance Level
                 campo_13 char(88) := lpad(' ',88,' '); --uso futuro

                 is_token char(1);
                 nro_cartao varchar2(50);
                 cd_bndr varchar2(50);
       begin
            ...
            select substr(a.tipo_layout, 3, 2),
                   ...
                   a.NRO_CARTAO_TE05,
                   b0bz.COD_BANDEIRA_B0
            into campo_1,
                 campo_6,
                 nro_cartao,
                 cd_bndr
            from tbl_input_massa_dados a
            inner join tbl_input_massa_dados_b0_bz b0bz on a.nro_identif_gera_massa = b0bz.nro_identif_gera_massa
            where a.nro_identif_gera_massa = p_numero_geracao_massa
                  and a.nro_linha_arquivo = p_nro_linha_arquivo;
            ...
            --Caso for bin token
            SELECT count(*) into is_token FROM CEM.TBCEMR_BIN token
                WHERE token.NU_BIN = substr(nro_cartao,1,6)
                AND cd_tipo_cpnt_sgrn = 1;

            if(is_token > 0) then
                begin
                    SELECT lpad(NU_CRTO_CRED_DNMC, 19, ' '),
                            lpad(NU_IDTF_RQSE_RECB, 11, ' '),
                            lpad(CD_NIVL_SGRN_RECB, 2, ' ')
                        INTO
                            campo_10,
                            campo_11,
                            campo_12
                        FROM CLC.TBCLCR_CRSA_CRTO_FSCO_DNMC
                        WHERE NU_CRTO_CRED_DNMC = nro_cartao --NUMERO CARTAO
                        AND   to_char(DT_LOCL_TRNS,'YYYY') || lpad(to_char(DT_LOCL_TRNS,'MM'),2,'0') || lpad(to_char(DT_LOCL_TRNS,'DD'),2,'0') =
                                            to_char(sysdate-1,'YYYY') || lpad(to_char(sysdate-1,'MM'),2,'0') || lpad(to_char(sysdate-1,'DD'),2,'0') --DATA VENDA
                        AND CD_BNDR = cd_bndr  --BANDEIRA
                        AND ROWNUM < 2;
                 exception
                          when no_data_found then
                               null;
      ...
      PROCEDURE LAYOUT_SUBTIPO_07 (p_numero_geracao_massa number,
       ...
       BEGIN
            ...
            -- cod motivo da transacao (B10)
            if (campo_5 = lpad('0',4,'0')) then
              begin
                select lpad(to_char(cd_mtvo_trne_fndo),4,'0')
                into campo_5
                from (SELECT cd_mtvo_trne_fndo
                FROM  CLC.TBCLCR_MTVO_TRNE_FNDO a 
                WHERE CD_BNDR = l_cod_bandeira 
                order by dbms_random.value)
                where rownum  = 1;            
              exception
            ...
            --valor origem (B05)
            if (campo_11 = lpad('0',12,'0')) then
                SELECT lpad(to_char(nvl((min(VL_MXMO_TRNE_FNDO) *100),0)),12,'0')
                into campo_11
                FROM  CLC.TBCLCR_MTVO_TRNE_FNDO a 
                WHERE CD_BNDR = l_cod_bandeira 
                and CD_MTVO_TRNE_FNDO = to_number(campo_5);
            end if;

            --codigo do destino (B12)
            if (campo_3 = lpad('0',4,'0')) then
              begin
                select lpad(to_char(cd_emsr),4,'0')
                into campo_3
                from (select cd_emsr
                     from (SELECT distinct CD_EMSR
                            FROM  CLC.TBCLCW_EMSR_RPLC
                            WHERE CD_BNDR = l_cod_bandeira )
                     order by dbms_random.value)
                where rownum  = 1;               
            ...
            -- codigo de origem (B11)
            if (campo_4 = lpad('0',4,'0')) then
               campo_4 := lpad(to_char(to_number(l_cod_adquir)),4,'0');
            end if;  

            if (campo_8 = lpad(' ',19,' ')) then
              begin
                  select bin
                  into l_nro_bin
                  from (SELECT to_char(NU_BIN) bin
                        ...
                        WHERE CD_BNDR = l_cod_bandeira
                           AND CD_TIPO_PLTF_PGMN = l_cod_tipo_plataforma
                           AND IN_TOKN = 'N'
                           and length(to_char(nu_bin)) = 6
                        order by dbms_random.value)
                  where rownum  = 1;

                  if (length(rtrim(ltrim(l_nro_bin))) <> 6) then
                      campo_8 := '9999999999999999   ';
                  else
                      campo_8 := rpad(rtrim(ltrim(l_nro_bin)) || '77' || '7777' || '7777',19,' ');
                  end if;

               exception
                        WHEN NO_DATA_FOUND THEN
                        campo_8 := '8888888888888888   ';
               end;
             end if;            

            if campo_1 in ('10') then
                l_valor_total_venda_debito := l_valor_total_venda_debito + to_number(campo_11);
                l_qtde_total_transacoes_debito := l_qtde_total_transacoes_debito + 1;
            end if;
            ...
            set cod_destino = nvl(cod_destino,campo_3) ,
                cod_origem = nvl(cod_origem,campo_4),
                cod_motivo_transacao = nvl(cod_motivo_transacao,campo_5),
                nro_cartao = nvl(nro_cartao,campo_8),
                vl_destino = nvl(vl_destino,campo_9),
                vl_origem = nvl(vl_origem,campo_11),
                dsc_mensagem_texto = nvl(dsc_mensagem_texto,campo_13)
            where nro_identif_gera_massa = p_numero_geracao_massa
                        and nro_linha_arquivo = p_nro_linha_arquivo;
       EXCEPTION
                when others then
       ...
       PROCEDURE LAYOUT_SUBTIPO_02_TE1020 (p_numero_geracao_massa number,
                 ...
                 campo_21 char(3) := lpad('1',3,'0'); --Quantidade de dias para liquidação financeira (23)
       ...
       BEGIN
            select substr(tipo_layout,3,2),
                   ...
                   lpad(nvl(dta_processamento,campo_22),8,'0'),
            ...
            update tbl_input_massa_dados_10_20 a
            set qtd_dias_liq_tran = nvl(qtd_dias_liq_tran,campo_21),
                dta_processamento = nvl(dta_processamento,campo_22),
                cod_token_pan = nvl(cod_token_pan,campo_24)
            where nro_identif_gera_massa = p_numero_geracao_massa
                        and nro_linha_arquivo = p_nro_linha_arquivo;
       EXCEPTION
                when others then
       ...
       BEGIN
            ...
            values (l_numero_geracao_massa,l_i,l_dsc_linha_arquivo);
             ...
             GERACAO_MASSA_10_20_TE (p_numero_geracao_massa,l_i);
       ...
       PROCEDURE GERACAO_MASSA_10_20_TE (p_numero_geracao_massa varchar2,l_i in out number) IS
            l_numero_geracao_massa number;
--            l_i number:=0;
    ...
    BEGIN
         ...
         LAYOUT_TE102000_CRITICAS(p_numero_geracao_massa);
         LAYOUT_TE102002_CRITICAS(p_numero_geracao_massa);
...
--           and m.nro_linha_arquivo = l_nro_linha_arquivo;
             and m.dsc_linha_arquivo like 'BZ%';
         exception
                  when DUP_VAL_ON_INDEX then
                       null;
         end;
         ...
         begin
           ...
           from (
            ...
            and (dsc_linha_arquivo like '0500%'
                or dsc_linha_arquivo like '2500%')
         ...
         begin
           ...
           from (
            ...
            from tbl_output_massa_dados a
            where nro_identif_gera_massa = p_numero_geracao_massa
            and (dsc_linha_arquivo like '0600%'
                or dsc_linha_arquivo like '2600%')
            and translate(substr(dsc_linha_arquivo,77,12),' +-0123456789.', ' ') is null);
         exception
                  when others then
                       l_teste_qtde := 0;
         ...
         -- Valor Total dos Transacoes de Debito Real
         begin
              select to_number(campo_5)
              into l_teste_numerico
              from tbl_output_massa_dados m
              where m.nro_identif_gera_massa = p_numero_geracao_massa
                    and m.nro_linha_arquivo = l_nro_linha_arquivo;
              if (l_teste_qtde <> campo_5) then
                l_retorno:= 'BZ:' || lpad(to_char(l_nro_linha_arquivo),4,'0') || lpad(':Regra 6.6 - 05:',20,' ') || lpad('Valor Total ads Transacoes de Movi de Parcelado:',45,' ') || campo_7 || ': Validacao Logica: Deve ser igual ao somatorio do valor das transacoes debito real.';
                PERSISTE_CRITICA(p_numero_geracao_massa,l_i,l_retorno);
              end if;
         exception
...
--         where teste_numerico is not null;
         ...
         begin
              select to_number(campo_4)
              into l_teste_numerico
              from tbl_output_massa_dados m
              where m.nro_identif_gera_massa = p_numero_geracao_massa
                    and m.nro_linha_arquivo = l_nro_linha_arquivo;
              if (l_teste_qtde <> campo_4) then
                l_retorno:= 'BZ:' || lpad(to_char(l_nro_linha_arquivo),4,'0') || lpad(':Regra 6.6 - 05:',20,' ') || lpad('Quantidadede Transacoes Debito Real:',45,' ') ||  campo_7 || ': Validacao Logica: Deve ser igual ao nro de registros do tipo 00.';
                PERSISTE_CRITICA(p_numero_geracao_massa,l_i,l_retorno);
              end if;
         exception
        ...
        exception
         ...
         for rc in (select distinct nro_linha_arquivo,dsc_linha_arquivo,
                    ...
                    from (select nro_linha_arquivo,dsc_linha_arquivo,
                          ...
                          from TBL_OUTPUT_MASSA_DADOS m
                          where substr(m.dsc_linha_arquivo, 1, 2) IN ('05', '06', '46','25','26')
                          AND substr(m.dsc_linha_arquivo, 3, 2) = '00'
            ...
            if (rc.valida_campo_11 is not null or
                ...
                l_retorno:= substr(rc.dsc_linha_arquivo, 1, 4) || ':' ||
         ...
         for rc in (select distinct m.nro_linha_arquivo,
                             ...
                             substr(rtrim(ltrim( substr(dsc_linha_arquivo,27,23))),2,6) bin,
                             case when ((bin.CD_BNDR is not null) or
                                       (bin2.CD_BNDR is not null and bin_token.CD_BNDR is not null))  then 1
                                  else 0 end valida_campo_7,
                             --MCC do PV - B16
                      ...
                      --Validacao cartao tokenizado
                      left join CLC.TBCLCW_BIN_RPLC  bin2
                      on bin2.NU_BIN = substr(rtrim(ltrim( substr(dsc_linha_arquivo,27,23))),2,6)
                         AND bin2.CD_BNDR = l_cod_bandeira
                         AND bin2.CD_TIPO_PLTF_PGMN = l_cod_tipo_plataforma
                         AND bin2.IN_TOKN = 'S'
                      left join CLC.tbclcr_crsa_crto_fsco_dnmc bin_token
                           on    bin_token.CD_BNDR = l_cod_bandeira --BANDEIRA
                           and   to_char(bin_token.DT_LOCL_TRNS,'YYYY') || lpad(to_char(bin_token.DT_LOCL_TRNS,'MM'),2,'0') || lpad(to_char(bin_token.DT_LOCL_TRNS,'DD'),2,'0') =
                                        to_char(sysdate-1,'YYYY') || lpad(to_char(sysdate-1,'MM'),2,'0') || lpad(to_char(sysdate-1,'DD'),2,'0') --DATA VENDA
                           and bin_token.NU_CRTO_CRED_DNMC = substr(dsc_linha_arquivo,5,19) --NUMERO CARTAO
                           and bin_token.NU_IDTF_RQSE_RECB is not null  -- Token Requestor ID
                           and bin_token.CD_NIVL_SGRN_RECB is not null  -- Token Assurance Level
                      ...
                      where m.nro_identif_gera_massa = p_numero_geracao_massa
                      and substr(m.dsc_linha_arquivo, 1, 2) IN  ('05', '06', '46','25','26')
                      and substr(m.dsc_linha_arquivo, 3, 2) = '00'
    ...
    BEGIN
         ...
         for rc in (select distinct nro_linha_arquivo,dsc_linha_arquivo,
                    ...
                    from (select nro_linha_arquivo,dsc_linha_arquivo,
                                 ...
                                 substr(dsc_linha_arquivo, 129, 1) campo_15, --Indicador de Movimentacao
                                 case when substr(dsc_linha_arquivo, 129, 1) in ('0', '1', '2') then 1 else 0 end valida_campo_15,
                          ...
                          from TBL_OUTPUT_MASSA_DADOS m
                          where substr(m.dsc_linha_arquivo, 1, 2) IN  ('05', '06', '46','25','26')
                          AND substr(m.dsc_linha_arquivo, 3, 2) = '01'
                          and m.nro_identif_gera_massa = p_numero_geracao_massa)
            ...
            if (rc.valida_campo_15 is not null) then
                l_retorno:= substr(rc.dsc_linha_arquivo, 1, 4) || ':' ||
                            lpad(to_char(rc.nro_linha_arquivo),4,'0') ||
                            lpad(':Regra 6.8 -B17:',20,' ') ||
                            rpad('Indicador de Movimentacao',45,' ') ||
                            lpad(rc.campo_15,20,' ') ||
                            ': Validacao Fisica: Indicador de Movimentacao deve ser numerico e igual aos domínios permitidos.';
                PERSISTE_CRITICA(p_numero_geracao_massa,l_i,l_retorno);
            end if;
            if (rc.valida_campo_16 is not null) then
                l_retorno:= substr(rc.dsc_linha_arquivo, 1, 4) || ':' ||
         ...
         for rc in (select distinct m.nro_linha_arquivo,
                             ...
                             substr(dsc_linha_arquivo, 129, 1) campo_15,
                             case when substr(m.dsc_linha_arquivo, 1, 2) = '46' and estq.CD_BNDR is null then 0 else 1 end valida_campo_15,
                      ...
                      left join CLC.TBCLCR_LNCM_ESTQ_TRNS estq
                           on estq.CD_BNDR = l_cod_bandeira
                           AND estq.CD_CRDE = l_cod_adquir
                           AND estq.CD_PRDT_BNDR = substr(dsc_linha_arquivo, 74, 3)
                           AND estq.CD_TIPO_PLTF_PGMN = l_cod_tipo_plataforma
                           AND estq.NU_RFRN = substr(dsc_linha_arquivo, 27, 23)
                           AND estq.NU_PRCL = substr(dsc_linha_arquivo, 133, 3)
                           AND estq.NU_BIN_RECB = substr(dsc_linha_arquivo, 5, 6)
                           AND estq.NU_FINL_CRTO_RECB = substr(dsc_linha_arquivo, 20, 4)
                           AND (
                               (substr(dsc_linha_arquivo, 129, 1) = '1'
                               AND estq.CD_TIPO_STCO_PRCL IN (1, 3))
                               OR
                               (substr(dsc_linha_arquivo, 129, 1) = '2'
                               AND estq.CD_TIPO_STCO_PRCL = 1)
                           )
            ...
            if (rc.valida_campo_15 = 0) then
                l_retorno:= substr(rc.dsc_linha_arquivo, 1, 4) || ':' ||
                            lpad(to_char(rc.nro_linha_arquivo),4,'0') ||
                            lpad(':Regra 6.8 -B18:',20,' ') ||
                            rpad('Indicador de Movimentação:',45,' ') ||
                            lpad(rc.campo_15,20,' ') ||
                            ': Validacao Logica: Transação deve existir na Elo';
                PERSISTE_CRITICA(p_numero_geracao_massa,l_i,l_retorno);
            end if;
    ...
    BEGIN
         ...
         for rc in (select distinct nro_linha_arquivo,dsc_linha_arquivo,
                    ...
                    from (select nro_linha_arquivo,dsc_linha_arquivo,
                          ...
                          from TBL_OUTPUT_MASSA_DADOS m
                          where substr(m.dsc_linha_arquivo, 1, 2) IN  ('05', '06', '46','25','26')
                          AND substr(m.dsc_linha_arquivo, 3, 2) = '02'
                          and m.nro_identif_gera_massa = p_numero_geracao_massa)
       ...
       PROCEDURE LAYOUT_TE102000_CRITICAS (p_numero_geracao_massa number ) IS
       l_dsc_linha_arquivo varchar2(500);
       l_retorno varchar2(400);
       l_i number := 1;
       l_saida number := 0;
       l_valor_maximo number := 0;
       l_codigo_bandeira number := 0;
        ...
        BEGIN
         ...
         if l_i is null then
            select 0 into l_i from dual;
         end if;
         
         PERSISTE_CRITICA(p_numero_geracao_massa,l_i,'----------------------');
         PERSISTE_CRITICA(p_numero_geracao_massa,l_i,'----------102000------');
         PERSISTE_CRITICA(p_numero_geracao_massa,l_i,'----------------------');

             -- pega bandeira  
         select substr(dsc_linha_arquivo,165,3)
            into l_codigo_bandeira
            from TBL_OUTPUT_MASSA_DADOS m
            where m.nro_identif_gera_massa = p_numero_geracao_massa
            and m.nro_linha_arquivo = 1;
         ...
         for rc in (select distinct nro_linha_arquivo,dsc_linha_arquivo,
                           campo_3, valida_campo_3,
                           campo_4, valida_campo_4,
                           campo_5, valida_campo_5,
                           campo_8, valida_campo_8,
                           campo_11, valida_campo_11
                    from (select nro_linha_arquivo,dsc_linha_arquivo,
                                 substr(m.dsc_linha_arquivo,5,4) campo_3, --codigo do destino
                                 translate(rtrim(ltrim( substr(m.dsc_linha_arquivo,5,4) )),' +-0123456789.', ' ') valida_campo_3,
                                 substr(m.dsc_linha_arquivo,9,4) campo_4, --codigo da origem
                                 translate(rtrim(ltrim( substr(m.dsc_linha_arquivo,9,4) )),' +-0123456789.', ' ') valida_campo_4,
                                 substr(m.dsc_linha_arquivo,13,4) campo_5, --motivo da transacao
                                 translate(rtrim(ltrim( substr(m.dsc_linha_arquivo,13,4) )),' +-0123456789.', ' ') valida_campo_5,
                                 substr(m.dsc_linha_arquivo,28,19) campo_8, --numero do cartao
                                 translate(rtrim(ltrim( substr(m.dsc_linha_arquivo,28,19) )),' +-0123456789.', ' ') valida_campo_8,

                                 substr(m.dsc_linha_arquivo,62,12) campo_11, --valor de origem
                                 translate(rtrim(ltrim( substr(m.dsc_linha_arquivo,62,12) )),' +-0123456789.', ' ') valida_campo_11
                          from TBL_OUTPUT_MASSA_DADOS m
                          where substr(m.dsc_linha_arquivo, 1, 2) IN  ('10', '20')
                          AND substr(m.dsc_linha_arquivo, 3, 2) = '00'
                          and m.nro_identif_gera_massa = p_numero_geracao_massa
                          order by nro_linha_arquivo)
                    where (valida_campo_3 is not null) or
                          (valida_campo_4 is not null) or
                          (valida_campo_5 is not null) or
                          (valida_campo_11 is not null or
                          (valida_campo_11 is null and to_number(campo_11) = 0) 
                          )
                   ) loop

            PERSISTE_CRITICA(p_numero_geracao_massa,l_i,rc.dsc_linha_arquivo);
            if (rc.valida_campo_3 is not null) then
                l_retorno:= substr(rc.dsc_linha_arquivo, 1, 4) || ':' ||
                            lpad(to_char(rc.nro_linha_arquivo),4,'0') ||
                            lpad(':Regra 7.8 -B11:',20,' ') ||
                            rpad('Codigo do destino:',25,' ') ||
                            lpad(rc.campo_3,20,' ') ||
                            ': Validacao Fisica: Numerico.';
                PERSISTE_CRITICA(p_numero_geracao_massa,l_i,l_retorno);
            end if;

            if (rc.valida_campo_4 is not null) then
                l_retorno:= substr(rc.dsc_linha_arquivo, 1, 4) || ':' ||
                            lpad(to_char(rc.nro_linha_arquivo),4,'0') ||
                            lpad(':Regra 7.8 -B11:',20,' ') ||
                            rpad('Codigo da origem:',25,' ') ||
                            lpad(rc.campo_4,20,' ') ||
                            ': Validacao Fisica: Numerico.';
                PERSISTE_CRITICA(p_numero_geracao_massa,l_i,l_retorno);
            end if;

            if (rc.valida_campo_5 is not null) then
                l_retorno:= substr(rc.dsc_linha_arquivo, 1, 4) || ':' ||
                            lpad(to_char(rc.nro_linha_arquivo),4,'0') ||
                            lpad(':Regra 7.8 -B10:',20,' ') ||
                            rpad('Codigo de motivo de transacao:',35,' ') ||
                            lpad(rc.campo_5,20,' ') ||
                            ': Validacao Fisica: Numerico.';
                PERSISTE_CRITICA(p_numero_geracao_massa,l_i,l_retorno);
            end if;
            ...
            if (rc.valida_campo_8 is not null) then
                l_retorno:= substr(rc.dsc_linha_arquivo, 1, 4) || ':' ||
                            lpad(to_char(rc.nro_linha_arquivo),4,'0') ||
                            lpad(':Regra 7.8 -B13:',20,' ') ||
                            rpad('Numero do cartao:',25,' ') ||
                            lpad(rc.campo_8,20,' ') ||
                            ': Validacao Fisica: Numerico.';
                PERSISTE_CRITICA(p_numero_geracao_massa,l_i,l_retorno);
            end if;
            ...
            if (rc.valida_campo_11 is not null or
                (rc.valida_campo_11 is null and to_number(rc.campo_11) = 0)
               ) then
                l_retorno:= substr(rc.dsc_linha_arquivo, 1, 4) || ':' ||
                            lpad(to_char(rc.nro_linha_arquivo),4,'0') ||
                            lpad(':Regra 7.8 -B05:',20,' ') ||
                            rpad('Valor de origem:',45,' ') ||
                            lpad(rc.campo_11,20,' ') ||
                            ': Validacao Fisica: Numerico e Diferente de zeros.';
                PERSISTE_CRITICA(p_numero_geracao_massa,l_i,l_retorno);
            end if;
         ...
         --critica logica
         for rc in (select nro_linha_arquivo,dsc_linha_arquivo,
                           substr(m.dsc_linha_arquivo,5,4) campo_3, --codigo do destino
                           translate(rtrim(ltrim( substr(m.dsc_linha_arquivo,5,4) )),' +-0123456789.', ' ') valida_campo_3,
                           substr(m.dsc_linha_arquivo,9,4) campo_4, --codigo da origem
                           translate(rtrim(ltrim( substr(m.dsc_linha_arquivo,9,4) )),' +-0123456789.', ' ') valida_campo_4,
         ...
         loop
            --codigo do destino
            if rc.valida_campo_3 is null then
               begin
               
                 SELECT 1
                 into l_saida
                 FROM CLC.TBCLCW_EMSR_RPLC
                 WHERE CD_BNDR = l_codigo_bandeira
                 AND CD_EMSR = to_number(rc.campo_3)
                 and rownum < 2;
               ...
               exception
                     when NO_DATA_FOUND then
                         l_retorno:= substr(rc.dsc_linha_arquivo, 1, 4) || ':' ||
                            lpad(to_char(rc.nro_linha_arquivo),4,'0') ||
                            lpad(':Regra 7.8 -B12:',20,' ') ||
                            rpad('Codigo do destino:',25,' ') ||
                            lpad(rc.campo_3,20,' ') ||
                            ': Validacao Logica: Emissor nao cadastrado.';
           ...
           --codigo da origem
           if rc.valida_campo_4 is null then
               begin
                   SELECT 1 
                   into l_saida
                   FROM CCR.TBCCRR_CRDE_BNDR 
                   WHERE CD_BNDR = l_codigo_bandeira AND CD_CRDE = to_number(rc.campo_4) AND IN_CRDE_ATVO = 'S' AND ROWNUM = 1;
               exception
                     when NO_DATA_FOUND then
                         l_retorno:= substr(rc.dsc_linha_arquivo, 1, 4) || ':' ||
                            lpad(to_char(rc.nro_linha_arquivo),4,'0') ||
                            ...
                            rpad('Codigo da origem:',25,' ') ||
                            lpad(rc.campo_4,20,' ') ||
                            ': Validacao Logica: Credenciadora nao cadastrada.';
           ...
           --valor de origem
           if rc.valida_campo_11 is null and rc.valida_campo_5 is null then
               begin
                   SELECT VL_MXMO_TRNE_FNDO 
                   into l_valor_maximo
                   FROM CLC.TBCLCR_MTVO_TRNE_FNDO 
                   WHERE CD_BNDR = l_codigo_bandeira AND CD_MTVO_TRNE_FNDO = to_number(rc.campo_5) AND ROWNUM < 2;
               exception
               when NO_DATA_FOUND then
                   l_valor_maximo := 0;
               end;
               --motivo da transacao
               if l_valor_maximo = 0 then
                   l_retorno:= substr(rc.dsc_linha_arquivo, 1, 4) || ':' ||
                               lpad(to_char(rc.nro_linha_arquivo),4,'0') ||
                               lpad(':Regra 7.8 -B10:',20,' ') ||
                               rpad('Motivo da transacao:',25,' ') ||
                               lpad(rc.campo_5,20,' ') ||
                               ': Validacao Logica: Motivo de transacao inexistente.';

                    PERSISTE_CRITICA(p_numero_geracao_massa,l_i,l_retorno);
              
               end if;

               if to_number(rc.campo_11) > l_valor_maximo then
                   l_retorno:= substr(rc.dsc_linha_arquivo, 1, 4) || ':' ||
                               lpad(to_char(rc.nro_linha_arquivo),4,'0') ||
                               lpad(':Regra 7.8 -B05:',20,' ') ||
                               rpad('Valor de origem:',25,' ') ||
                               lpad(rc.campo_11,20,' ') ||
                               ': Validacao Logica: Valor de origem superior ao limite.';
           ...
           --cartao de credito
           if rc.valida_campo_8 is null then
               begin
                   select 1
                   into l_valor_maximo
                   from CLC.TBCLCW_BIN_RPLC
                   where nu_bin = to_number(substr(rc.campo_8,1,6));
               exception
               when NO_DATA_FOUND then
                   l_retorno:= substr(rc.dsc_linha_arquivo, 1, 4) || ':' ||
                               lpad(to_char(rc.nro_linha_arquivo),4,'0') ||
                               lpad(':Regra 7.8 -B13:',20,' ') ||
                               rpad('Numero do cartao:',30,' ') ||
                               lpad(rc.campo_8,20,' ') ||
                               ': Validacao Logica: BIN do cartao nao cadastrado.';
       ...
       PROCEDURE LAYOUT_TE102002_CRITICAS (p_numero_geracao_massa number ) IS
       l_dsc_linha_arquivo varchar2(500);
       l_retorno varchar2(400);
       l_i number := 1;
        ...
        BEGIN
         ...
         PERSISTE_CRITICA(p_numero_geracao_massa,l_i,'----------------------');
         PERSISTE_CRITICA(p_numero_geracao_massa,l_i,'----------102002------');
         PERSISTE_CRITICA(p_numero_geracao_massa,l_i,'----------------------');
         ...
         for rc in (select distinct nro_linha_arquivo,dsc_linha_arquivo,
                    ...
                    from (select nro_linha_arquivo,dsc_linha_arquivo,
                          ...
                          order by nro_linha_arquivo)
                    where (valida_campo_6 is not null or
                          (valida_campo_6 is null and to_number(campo_6) = 0) 
                          )
                   ) loop
            ...
            if (rc.valida_campo_6 is not null or
                ...
                l_retorno:= substr(rc.dsc_linha_arquivo, 1, 4) || ':' ||
                            lpad(to_char(rc.nro_linha_arquivo),4,'0') ||
                            lpad(':Regra 7.8 -B04:',20,' ') ||
                            rpad('Qtde de dias para liquidacao:',40,' ') ||
                            lpad(rc.campo_6,20,' ') ||
                            ': Validacao Fisica: Numerico e Diferente de zeros.';
                PERSISTE_CRITICA(p_numero_geracao_massa,l_i,l_retorno);
            end if;
         ...
         -- nao ha consistencia logica para o campo 6
        end;
